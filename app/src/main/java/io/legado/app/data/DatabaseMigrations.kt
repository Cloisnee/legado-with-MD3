...
object DatabaseMigrations {
    ...
    // v4-B9：book_content_processes 降列（拔除 aiArtifactId / sourceContentHash；表重建式手写迁移，
    // 自动迁移需历史 schema 106.json，本仓快照止于 105）
    private val migration_106_107 = object : Migration(106, 107) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `book_content_processes_new` (
                    `id` TEXT NOT NULL,
            ...
            db.execSQL(
                ...
                INSERT INTO `book_content_processes_new` (
                    `id`, `bookUrl`, `chapterIndex`, `kind`, `stage`, `target`,
                    `anchorJson`, `actionJson`, `styleJson`, `source`, `enabled`,
                    `sortOrder`, `status`, `schemaVersion`, `createdAt`, `updatedAt`
                )
                SELECT `id`, `bookUrl`, `chapterIndex`, `kind`, `stage`, `target`,
                    `anchorJson`, `actionJson`, `styleJson`, `source`, `enabled`,
                    `sortOrder`, `status`, `schemaVersion`, `createdAt`, `updatedAt`
            ...
            db.execSQL(
                "CREATE INDEX IF NOT EXISTS `index_book_content_processes_bookUrl_chapterIndex_enabled_sortOrder` " +
